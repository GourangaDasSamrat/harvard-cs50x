# CS50: Introduction to Computer Science

This is **CS50**, Harvard University's introduction to the intellectual enterprises of computer science and the art of programming.

---

## Disclaimer

> A quick word on [Academic Honesty](https://cs50.harvard.edu/x/2026/honesty/).
> While these solutions may help you navigate the course, you should first try to solve each problem set yourself. Investing time and energy into figuring out solutions independently is the best way to achieve mastery.
> Go ahead, peek at the solutions if needed, and most importantly, have fun.

---

## CS50x 2026 - Introduction to Computer Science

This repository contains my **labs and problem sets** for Harvard's [CS50x 2026](https://cs50.harvard.edu/x/2026/).

> Clicking on the name of a project will take you to **my solution**, while clicking on _Lab_ or _PSet Instructions_ will take you to **CS50's official instructions and guidelines**.

#### Every lab and problem set submitted achieved **100%**.

If you have questions, feel free to ask on [Discussions](https://github.com/GourangaDasSamrat/harvard-cs50x/discussions).
Please leave a ⭐ star if you find this repository helpful!

---

## Table of Contents

- [Week 0: Scratch](./Week%200/Problem%20Set%200/RoamingCat.sb3) - [PSet Instructions](https://cs50.harvard.edu/x/psets/0/)

- [Week 1: C](./Week%201/Problem%20Set%201/) - [PSet Instructions](https://cs50.harvard.edu/x/psets/1/)

  - [me](./Week%201/Problem%20Set%201/me/hello.c)
  - mario
    - [more](./Week%201/Problem%20Set%201/mario-more/mario.c)
  - [credit](./Week%201/Problem%20Set%201/credit/credit.c)

- [Week 2: Arrays](./Week%202/Problem%20Set%202/) - [PSet Instructions](https://cs50.harvard.edu/x/psets/2/)
  - [scrabble](./Week%202/Problem%20Set%202/scrabble/scrabble.c)
  - [readability](./Week%202/Problem%20Set%202/readability/readability.c)
  - [caesar](./Week%202/Problem%20Set%202/caesar/caesar.c)

---

## Acknowledgements

This course would not be possible without the incredible work of:

- **[Harvard University](https://www.harvard.edu/)** - For making CS50 freely available to learners worldwide
- **[David J. Malan](https://www.linkedin.com/in/malan/)** - Professor of Practice at Harvard University, who teaches the lectures with unmatched passion and clarity
- **[Kelly Ding](https://www.linkedin.com/in/kellyding)** - Preceptor of Computer Science at Harvard University, who leads the sections and provides invaluable guidance

Their dedication to education and making computer science accessible to everyone is truly inspiring.

---

_This repository is maintained for educational purposes and personal learning._
