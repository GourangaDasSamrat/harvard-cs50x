...
unsigned int hash(const char* word) {
  ...
  // consider a function on each character in a word to insurace uniqueness
  // however will output fit in N-1? to fit N do module % N output is the
  // address in the linked list return toupper(word[0]) - 'A';
...
bool load(const char* dictionary) {
  ...
  FILE* dict_open =
    fopen(dictionary, "r");  // pointer to the memory address of the input file
...
bool unload(void) {
  // TODO
  // tmp and curosr method move cursor->cursor->next. free temp..
  // cursor->next...NULL loop along the index positions
  for (int i = 0; i < N; i++) {
    ...
    // traverse the linked list at that index position and free - use
    // temp/cursor while loop until NULL at that point next i will begin
    while (temp != NULL) {
      // set cursor to next
